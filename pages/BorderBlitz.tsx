...
import { Timer, Trophy, ArrowLeft, MapPin, Play, Lock, Crown, Check, X } from 'lucide-react';
...
export default function BorderBlitz() {
  const [gameState, setGameState] = useState<'start' | 'playing' | 'finished'>('start');
  const [score, setScore] = useState(0);
  const [timeLeft, setTimeLeft] = useState(60);
  const [currentQuestion, setCurrentQuestion] = useState<{ country: Country; options: string[]; correctBorders: string[] } | null>(null);
  ...
  const countriesWithBorders = useMemo(() => {
    return MOCK_COUNTRIES.filter(c => c.borders && c.borders.length > 0);
  }, []);

  // Get all country names for distractors
  const allCountryNames = useMemo(() => {
    return MOCK_COUNTRIES.map(c => c.name);
  }, []);
  ...
  const generateQuestion = useCallback(() => {
    ...
    const country = countriesWithBorders[Math.floor(Math.random() * countriesWithBorders.length)];
    const correctBorders = country.borders || [];
    
    // Pick one correct border
    const correctAnswer = correctBorders[Math.floor(Math.random() * correctBorders.length)];
    
    // Get 3 non-bordering countries as distractors
    const distractors = allCountryNames
      .filter(name => !correctBorders.includes(name) && name !== country.name)
      .sort(() => Math.random() - 0.5)
      .slice(0, 3);
    
    const options = shuffle([correctAnswer, ...distractors]);
    setCurrentQuestion({ country, options, correctBorders });
  }, [countriesWithBorders, allCountryNames]);
  ...
  const handleAnswer = (answer: string) => {
    ...
    const isCorrect = currentQuestion.correctBorders.includes(answer);
  ...
  if (!isPremium && gameState === 'start') {
    return (
      <div className="h-[100dvh] min-h-screen bg-surface-dark font-sans relative overflow-hidden flex items-center justify-center px-4">
        <SEO title="Border Blitz - Premium Game" description="Race to identify bordering countries. A premium geography game." />
        ...
        <div className="max-w-md w-full bg-white/10 backdrop-blur-3xl rounded-3xl p-8 text-center border-2 border-amber-500/30 relative z-10">
          ...
          <h1 className="text-4xl font-display font-black text-white mb-2 uppercase tracking-tighter">Border Blitz</h1>
      ...
      <AnimatePresence mode="wait">
        {gameState === 'start' && (
          ...
          >
            ...
            <div className="fixed inset-0 z-0 pointer-events-none">
              <div className="absolute top-[-20%] left-[-10%] w-[100%] h-[100%] bg-error/20 rounded-full blur-[150px] opacity-60" />
            ...
            <div className="max-w-md w-full bg-white/10 backdrop-blur-3xl rounded-3xl p-8 text-center border-2 border-white/20 relative z-10">
              ...
              <div className="w-20 h-20 bg-error/20 rounded-2xl flex items-center justify-center mx-auto mb-8 text-error border border-white/30">
                <MapPin size={36} />
              </div>
              <h1 className="text-4xl font-display font-black text-white mb-2 uppercase tracking-tighter">Border Blitz</h1>
              <p className="text-white/40 text-[10px] mb-10 font-bold uppercase tracking-[0.2em]">Identify bordering countries.</p>
        ...
        {gameState === 'playing' && currentQuestion && (
          ...
          >
            <SEO title="Border Blitz - Playing" description="Race to identify bordering countries." />
            
            <div className="fixed inset-0 z-0 pointer-events-none">
              <div className="absolute top-[10%] right-[10%] w-[60%] h-[60%] bg-error/10 rounded-full blur-[100px]" />
            ...
            <div className="max-w-3xl mx-auto w-full flex shrink-0 items-center gap-2 mb-3 md:mb-4 bg-white/10 backdrop-blur-2xl p-2.5 md:p-3 rounded-2xl border border-white/20 z-10">
              ...
              <div className="flex-1 flex flex-col items-center justify-center min-w-0">
                <h1 className="text-[9px] sm:text-[10px] font-black text-white uppercase tracking-[0.15em] sm:tracking-[0.3em] truncate">Border Blitz</h1>
                <div className="h-0.5 w-6 bg-error/40 rounded-full mt-1" />
              </div>
            ...
            <div className="flex-1 max-w-3xl mx-auto w-full flex flex-col min-h-0 bg-white/10 backdrop-blur-3xl rounded-2xl md:rounded-3xl border border-white/20 overflow-hidden relative z-10 p-4 md:p-6">
              ...
              <div className="flex-1 flex flex-col items-center justify-center">
                <div className="mb-6 text-center">
                  ...
                  <p className="text-error text-sm font-bold uppercase tracking-[0.2em] flex items-center justify-center gap-2">
                    <MapPin size={16} /> Which country borders this?
                  </p>
                  <p className="text-white/40 text-xs mt-2">
                    {currentQuestion.correctBorders.length} bordering {currentQuestion.correctBorders.length === 1 ? 'country' : 'countries'}
                  </p>
                </div>

                <div className="w-full max-w-lg grid grid-cols-1 sm:grid-cols-2 gap-3">
                  {currentQuestion.options.map((option, idx) => {
                    const isCorrect = currentQuestion.correctBorders.includes(option);
