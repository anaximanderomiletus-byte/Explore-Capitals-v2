...
export default function LandmarkLegend() {
  const [gameState, setGameState] = useState<'start' | 'preparing' | 'playing' | 'finished'>('preparing');
      ...
      <AnimatePresence mode="wait">
        ...
        {gameState === 'preparing' && (
          ...
          >
            <div className="text-center relative z-10">
              <h2 className="text-white font-display font-black text-2xl uppercase tracking-[0.5em] animate-pulse">Loading</h2>
              <button 
                onClick={() => navigate('/games')}
                className="inline-flex items-center justify-center gap-2 text-white/30 hover:text-white transition-all font-black uppercase tracking-[0.3em] text-[10px] group mt-12 relative z-20 pointer-events-auto"
              >
                <ArrowLeft size={14} className="group-hover:-translate-x-1 transition-transform" /> 
                Back
              </button>
        ...
        {gameState === 'playing' && currentQuestion && (
          ...
          >
            ...
            <div className="max-w-5xl mx-auto w-full flex shrink-0 items-center justify-between mb-4 bg-white/10 backdrop-blur-2xl p-4 rounded-3xl shadow-glass border-2 border-white/20 mt-16 md:mt-20 relative overflow-hidden z-10">
               <div className="absolute inset-0 bg-glossy-gradient opacity-10" />
               ...
               <div className="absolute left-1/2 -translate-x-1/2 flex flex-col items-center">
                  <h1 className="text-[10px] md:text-xs font-black text-white uppercase tracking-[0.3em] drop-shadow-md">Landmark Legend</h1>
            ...
            <div className="flex-1 max-w-5xl mx-auto w-full flex flex-col md:flex-row gap-5 min-h-0 bg-white/10 backdrop-blur-3xl rounded-3xl border-2 border-white/20 p-5 md:p-8 overflow-hidden relative shadow-glass z-10">
               ...
               <AnimatePresence mode="wait">
                 <motion.div
                   ...
                   className="flex-1 flex flex-col md:flex-row gap-5 min-h-0 relative z-10"
                 >
                   <div className="flex-1 flex flex-col min-h-0 relative z-10">
                      <div className="relative flex-1 rounded-[2rem] overflow-hidden bg-black/40 border-2 border-white/10 shadow-inner group min-h-[220px] md:min-h-0">
                          <img 
                          ...
                          <div className="absolute bottom-4 left-4 right-4 p-5 bg-surface-dark/80 backdrop-blur-2xl rounded-[1.8rem] border-2 border-white/40 text-white flex items-center gap-4 shadow-[0_20px_50px_rgba(0,0,0,0.5)] overflow-hidden z-20">
                             <div className="absolute inset-0 bg-glossy-gradient opacity-15 pointer-events-none rounded-[inherit]" />
                             <div className="p-2 bg-sky/30 rounded-xl relative z-10 shadow-glow-sky border border-white/20"><MapPin size={24} className="text-sky-light drop-shadow-glow-sky" /></div>
                             <h2 className="font-display font-black text-xl md:text-3xl drop-shadow-2xl relative z-10 uppercase tracking-tighter leading-none">{currentQuestion.landmarkName}</h2>
                          </div>
                      </div>
                   </div>

                   <div className="w-full md:w-80 flex flex-col justify-center gap-2.5 shrink-0 pb-2 relative z-10">
                      <p className="text-sky font-black text-[9px] uppercase tracking-[0.4em] text-center mb-1 font-sans drop-shadow-glow-sky">IDENTIFY MISSION TARGET</p>
                      {currentQuestion.options.map((option) => {
                        const isSelected = selectedAnswerId === option.id;
                        const isCorrect = option.id === currentQuestion.country.id;
                        const isWrong = isSelected && !isCorrect;
                        
                        let stateStyles = "bg-white/10 border-2 border-white/40 text-white hover:bg-white/20 hover:border-sky/50 shadow-glass-bubble relative overflow-hidden group/opt";
                        ...
                        return (
                          <button
                            ...
                            className={`relative p-3.5 rounded-[1.5rem] font-display font-black text-base md:text-lg flex items-center justify-between min-h-[64px] md:min-h-[72px] transition-all duration-500 uppercase tracking-tighter group ${stateStyles} ${isWrong ? 'animate-shake' : ''}`}
                          ...
                          >
                            ...
                            <span className="text-left leading-tight truncate pr-3 relative z-10 drop-shadow-md">{option.name}</span>
                            <img 
                              src={`https://flagcdn.com/w80/${getCountryCode(option.flag)}.png`}
                              alt={`${option.name} Flag`}
                              className="h-6 md:h-8 w-auto relative z-10 drop-shadow-lg object-contain"
                            />
                          </button>
                        );
                      })}
