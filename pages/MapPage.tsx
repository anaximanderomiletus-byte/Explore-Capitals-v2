...
const MapPage: React.FC = () => {
  ...
  useEffect(() => {
    isLoadingRandomRef.current = isLoadingRandom;
  }, [isLoadingRandom]);
  ...
  const createPopupContent = useCallback(
    (country: Country, type: "sovereign" | "territory" | "defacto") => {
      const flagUrl = getFlagUrl(country.flag);

      let subheader = "";
      let linkClass = "text-text";
      let linkGlow = "";
      ...
      if (type === "territory") {
        ...
        subheader = `<div class="text-[10px] font-black text-text uppercase tracking-[0.2em] mb-3">${t("map.sovereignState")}</div>`;
      }
 ...
 <img src="${flagUrl}" alt="${country.name} Flag" class="w-full h-full object-contain filter " />
 </div>
 <h3 class="font-display font-black text-2xl text-text tracking-tighter uppercase leading-none m-0 ">${country.name}</h3>
 </div>

 ${subheader}

 <div class="mb-6 bg-surface p-4 rounded-2xl border border-border relative text-center">
 <span class="text-[10px] font-black text-primary uppercase tracking-[0.2em] block mb-1">${t("map.capitalCity")}</span>
 <span class="font-display font-black text-text text-lg uppercase tracking-tight ">${country.capital}</span>
 </div>

 <div class="text-center relative">
 <button
 data-country-slug="${toSlug(country.name)}"
 class="learn-more-btn ${linkClass} ${linkGlow} w-full h-12 bg-accent-soft border border-border rounded-2xl text-[11px] font-black uppercase tracking-[0.3em] hover:bg-accent-soft transition-all outline-none"
 >
 ${t("map.viewCountry")}
 </button>
 </div>
 ...
 `;
  ...
  const centerMapOnMarker = useCallback((marker: any) => {
    ...
    const targetY =
      topNavHeight + (mapSize.y - topNavHeight - bottomSheetHeight) / 2;
    ...
    const newCenterGlobal = markerGlobal.subtract(
      L.point(screenOffsetX, screenOffsetY),
    );
  ...
  useEffect(() => {
    if (regionScrollRef.current && selectedRegion) {
      const activeButton = regionScrollRef.current.querySelector(
  ...
  useEffect(() => {
    ...
    requestAnimationFrame(() => {
      ...
      if (regionChanged) {
        ...
        > = {
          Africa: {
            center: isMobile ? [5, 20] : [0, 50],
            zoom: isMobile ? 2.3 : 3.2,
          },
          Asia: {
            center: isMobile ? [35, 95] : [30, 115],
            zoom: isMobile ? 1.8 : 3.0,
          },
          Europe: {
            center: isMobile ? [52, 15] : [52, 45],
            zoom: isMobile ? 3.0 : 3.8,
          },
          "North America": {
            center: isMobile ? [40, -100] : [40, -75],
            zoom: isMobile ? 2.0 : 3.0,
          },
          "South America": {
            center: isMobile ? [-20, -60] : [-20, -35],
            zoom: isMobile ? 2.2 : 3.2,
          },
          Oceania: {
            center: isMobile ? [-25, 145] : [-15, 180],
            zoom: isMobile ? 2.3 : 3.2,
          },
          All: {
            center: isMobile ? [20, 0] : [20, 25],
  ...
  useEffect(() => {
    ...
    if (countryId && mapReady && mapInstanceRef.current) {
      ...
      if (stored) {
        ...
        if (stored.type === "territory" && !showTerritories)
        ...
        setTimeout(() => {
          ...
          const markers = allMarkersRef.current.filter(
            (m) => m.id === countryId,
          );
          ...
          setTimeout(() => {
            ...
            if (targetMarker.marker.getPopup())
              targetMarker.marker.getPopup().update();
          }, 550);
        }, 100);
      }
    } else {
  ...
  const flyToRandom = () => {
    ...
    if (mapInstanceRef.current && markers.length > 0) {
      ...
      const isZoomCorrect =
        Math.abs(mapInstanceRef.current.getZoom() - targetZoom) < 0.1;
  ...
  const filteredSearchResults = useMemo(() => {
    ...
    return [...COUNTRIES, ...TERRITORIES, ...DE_FACTO_COUNTRIES]
      .filter(
        (c) =>
  ...
  const handleResultClick = (country: Country) => {
    ...
    if (firstMarker) {
      if (firstMarker.type === "territory" && !showTerritories)
        setShowTerritories(true);
  ...
  return (
    <div className="fixed inset-0 z-40 bg-surface animate-[fadeIn_0.3s_ease-out_both]">
      <SEO
      ...
      <div
        ...
        style={{ background: "#E9EEF3", height: "100%", width: "100%" }}
      >
        {!mapReady && (
          <div className="absolute inset-0 flex items-center justify-center text-muted font-display font-black uppercase tracking-widest pointer-events-none">
      ...
      <div
        className={`fixed bottom-10 left-6 z-[2000] pointer-events-auto hidden md:block transition-all duration-700 ease-out ${showUI ? "opacity-0 pointer-events-none translate-y-10" : "opacity-100 translate-y-0"}`}
      >
        <button
          onClick={() => setShowUI(true)}
          className="w-12 h-12 bg-elevated border border-border rounded-full flex items-center justify-center text-text hover:bg-surface transition-all relative overflow-hidden group"
          title={t("map.showUI")}
        >
      ...
      <div
        className={`fixed z-[2000] pointer-events-auto md:hidden transition-all duration-700 ease-out ${showUI ? "opacity-0 pointer-events-none" : "bottom-10 left-6 opacity-100"}`}
      >
        <button
          onClick={() => setShowUI(!showUI)}
          className="w-12 h-12 bg-elevated border border-border rounded-full flex items-center justify-center text-text hover:bg-surface transition-all relative overflow-hidden group shadow-premium"
          title={t("map.showUI")}
        >
          <Eye size={20} className="relative z-10" />
        </button>
      ...
      {/* --- DESKTOP UI --- */}
      <div
        className={`hidden md:flex absolute top-24 left-6 w-60 flex-col gap-3 z-[1000] pointer-events-none transition-all duration-700 ease-out transform ${showUI ? "translate-x-0 opacity-100" : "-translate-x-full -translate-y-10 opacity-0"}`}
      >
        {/* Search Bar */}
        <div className="pointer-events-auto relative z-50">
          <div className="bg-elevated rounded-xl border border-border shadow-premium flex items-center px-3.5 py-2.5 focus-within:ring-4 focus-within:ring-primary/10 focus-within:border-border group relative overflow-hidden">
            <button
              onClick={() =>
                filteredSearchResults.length > 0 &&
                handleResultClick(filteredSearchResults[0])
              }
              className="mr-3 text-muted hover:text-primary transition-all focus:outline-none relative z-10"
            >
              <Search size={16} />
            </button>
            <input
              ...
              className="bg-transparent border-none outline-none text-[9px] text-text w-full placeholder:text-muted font-black uppercase tracking-[0.2em] relative z-10"
              value={searchQuery}
            ...
            {searchQuery && (
              <button
                ...
                className="text-muted hover:text-text relative z-10 p-1"
              >
                <X size={16} />
              </button>
            )}
          ...
          {searchQuery && (
            <div
              ...
              className="absolute left-0 w-full bg-elevated shadow-premium rounded-xl border border-border overflow-y-auto custom-scrollbar animate-in fade-in z-[60] overflow-hidden max-h-[50vh] top-full mt-2 slide-in-from-top-2"
            >
              {filteredSearchResults.length > 0 ? (
                <ul className="py-1 relative z-10">
                  {filteredSearchResults.map((country, index) => {
                    const isTerritory = TERRITORIES.some(
                      (t) => t.id === country.id,
                    );
                    const isDeFacto = DE_FACTO_COUNTRIES.some(
                      (d) => d.id === country.id,
                    );
                    ...
                    return (
                      <li key={country.id}>
                        <button
                          ...
                          className={`w-full text-left px-3.5 py-2.5 flex items-center gap-3 transition-all border-b border-border last:border-none uppercase tracking-tighter ${index === selectedIndex ? "bg-accent-soft scale-[1.02] translate-x-1" : "hover:bg-surface"}`}
                        >
                          ...
                          <div>
                            <p
                              className={`text-[10px] font-black ${nameClass} ${glowClass}`}
                            >
                              {country.name}
                            </p>
                            <p className="text-[8px] font-bold text-muted tracking-widest whitespace-nowrap">
                              {country.region}
                            </p>
                          </div>
                        </button>
                ...
                <div className="p-8 text-center text-[9px] text-muted font-black uppercase tracking-[0.3em] relative z-10">
                  No matches found
                </div>
              )}
            </div>
        ...
        {/* Region Card */}
        <div className="bg-elevated p-4 rounded-2xl border border-border shadow-premium pointer-events-auto relative overflow-hidden">
          <div className="flex items-center gap-2.5 mb-5 relative z-10">
            <div className="p-2 bg-primary/10 rounded-xl text-primary border border-primary/20">
              <MapIcon size={18} />
            </div>
            <div>
              <h1 className="font-display font-black text-text leading-none text-base uppercase tracking-tighter">
                Atlas
              </h1>
              <p className="text-[8px] text-muted font-black uppercase tracking-[0.2em] mt-1">
                {COUNTRIES.length +
                  TERRITORIES.length +
                  DE_FACTO_COUNTRIES.length}{" "}
                Countries
              </p>
            </div>
          </div>
          ...
          <div className="space-y-4 relative z-10">
            <div className="space-y-2.5">
              <div className="flex items-center gap-2 text-[8px] font-black text-muted uppercase tracking-[0.4em] mb-1 ml-1">
                <Filter size={10} />
                <span>Sectors</span>
              </div>
                  ...
                  <button
                    ...
                    className={`px-3 py-1.5 text-[8px] font-black uppercase tracking-widest rounded-lg transition-all duration-500 border-2 whitespace-nowrap ${
                      selectedRegion === region
                        ...
                        : "bg-surface text-muted border-border hover:bg-surface hover:text-text"
                    }`}
                  >
                    {region}
                  </button>
            ...
            <div className="pt-4 border-t border-border flex flex-col gap-2">
              <button
                ...
                className={`w-full flex items-center justify-between px-3 py-2 rounded-xl transition-all duration-500 text-[8px] font-black uppercase tracking-[0.2em] shadow-inner border ${showTerritories ? "bg-accent/20 text-primary border-primary/40" : "bg-surface text-muted border-border opacity-40 hover:opacity-100 hover:bg-surface"}`}
              >
                <span className="flex items-center gap-2">
                  <Globe size={12} /> Territories
                </span>
                <div
                  className={`w-7 h-3.5 rounded-full p-0.5 transition-all duration-500 ${showTerritories ? "bg-accent" : "bg-accent-soft"}`}
                >
                  <div
                    className={`w-2.5 h-2.5 bg-elevated rounded-full shadow-md transform transition-transform duration-500 ${showTerritories ? "translate-x-3.5" : "translate-x-0"}`}
                  ></div>
                </div>
              </button>
              ...
              <button
                ...
                className={`w-full flex items-center justify-between px-3 py-2 rounded-xl transition-all duration-500 text-[8px] font-black uppercase tracking-[0.2em] shadow-inner border ${showDeFacto ? "bg-warning/20 text-warning border-warning/40" : "bg-surface text-muted border-border opacity-40 hover:opacity-100 hover:bg-surface"}`}
              >
                <span className="flex items-center gap-2">
                  <AlertTriangle size={12} /> {t("map.deFact")}
                </span>
                <div
                  className={`w-7 h-3.5 rounded-full p-0.5 transition-all duration-500 ${showDeFacto ? "bg-warning" : "bg-accent-soft"}`}
                >
                  <div
                    className={`w-2.5 h-2.5 bg-elevated rounded-full shadow-md transform transition-transform duration-500 ${showDeFacto ? "translate-x-3.5" : "translate-x-0"}`}
                  ></div>
                </div>
              </button>
            </div>
        ...
        <div className="bg-elevated p-3.5 rounded-2xl border border-border shadow-premium pointer-events-auto relative overflow-hidden">
          <div className="flex flex-col gap-2 text-[8px] font-black uppercase tracking-[0.3em] text-muted relative z-10">
            <div className="flex items-center gap-2">
              <div className="w-2 h-2 rounded-full bg-primary border border-border"></div>
              <span className="">{t("map.sovereign")}</span>
            </div>
            <div className="flex items-center gap-2">
              <div className="w-2 h-2 rounded-full bg-accent border border-border"></div>
              <span className="">{t("map.territory")}</span>
            </div>
            <div className="flex items-center gap-2">
              <div className="w-2 h-2 rounded-full bg-warning border border-border"></div>
              <span className="">{t("map.deFact")}</span>
            </div>
          </div>
        </div>
      </div>
      ...
      <div
        className={`hidden md:flex fixed bottom-10 left-0 right-0 justify-center z-[1001] pointer-events-none transition-all duration-700 ease-out ${showUI ? "translate-y-0 opacity-100" : "translate-y-40 opacity-0"}`}
      >
        <div className="flex items-center gap-4 pointer-events-auto">
          ...
          <button
            ...
            title={t("map.hideUI")}
          >
            <EyeOff size={20} className="relative z-10" />
          </button>
          ...
          <button
            ...
            className="flex items-center gap-4 px-10 h-14 text-lg rounded-full bg-elevated border border-border shadow-premium uppercase tracking-[0.15em] group relative overflow-hidden transition-all duration-500 text-text font-display font-black disabled:opacity-40 disabled:cursor-not-allowed disabled:saturate-50 hover:bg-surface whitespace-nowrap"
          >
            <div className="relative z-10 flex items-center gap-3">
              <Compass
                size={24}
                className={`transition-transform duration-1000 ${isLoadingRandom || !mapReady ? "animate-spin text-primary" : "text-primary"}`}
              />
          ...
          <div className="hidden lg:flex items-center gap-2">
            <button
              ...
              className="w-12 h-12 bg-elevated border border-border rounded-full flex items-center justify-center text-text hover:bg-surface transition-all group relative overflow-hidden"
            >
      ...
      {/* --- MOBILE UI --- */}
      <div
        className={`md:hidden fixed bottom-0 left-0 w-full z-[1000] flex flex-col items-center pointer-events-none transition-all duration-700 ease-out transform ${showUI ? "translate-y-0 opacity-100" : "translate-y-full opacity-0"}`}
      >
        {/* Action Row */}
        <div className="w-full px-4 pb-4 flex items-end justify-between pointer-events-none">
          <button
            onClick={() => setShowUI(false)}
            className="pointer-events-auto w-12 h-12 bg-elevated border border-border rounded-full flex items-center justify-center text-text hover:bg-surface transition-all relative overflow-hidden flex-shrink-0 group shadow-premium"
          >
            <EyeOff size={20} className="relative z-10" />
          </button>
          ...
          <div className="flex-1 flex justify-center px-2">
            <button
              ...
              className="pointer-events-auto bg-elevated text-text border border-border px-8 py-3.5 rounded-full flex items-center gap-3 transition-all group relative overflow-hidden disabled:opacity-40 disabled:saturate-50 hover:bg-surface shadow-premium"
            >
              <div className="relative z-10 flex items-center gap-3">
                <Compass
                  size={20}
                  className={`transition-transform duration-1000 ${isLoadingRandom || !mapReady ? "animate-spin text-primary" : "text-primary"}`}
                />
                <span className="font-display font-black text-xs uppercase tracking-[0.1em] whitespace-nowrap">
                  {!mapReady
                    ? "INITIALIZING..."
                    : isLoadingRandom
                      ? "SEARCHING..."
          ...
          <div className="flex flex-col gap-2 flex-shrink-0">
            <button
              onClick={handleZoomIn}
              className="pointer-events-auto w-12 h-12 bg-elevated border border-border rounded-full flex items-center justify-center text-text hover:bg-surface transition-all relative overflow-hidden group shadow-premium"
            >
        ...
        <div
          className="w-full pointer-events-auto bg-elevated border-t border-x border-border shadow-premium rounded-t-3xl p-4 flex flex-col gap-3 relative overflow-hidden"
          style={{
        ...
        >
          ...
          <div className="relative z-20">
            <div className="relative group bg-surface rounded-xl border border-border focus-within:bg-surface focus-within:ring-2 focus-within:ring-primary/20 transition-all duration-500 overflow-hidden">
              ...
              <input
                type="text"
                placeholder={t("map.search")}
                className="block w-full pl-12 pr-12 py-3 bg-transparent text-text text-xs font-black uppercase tracking-widest placeholder:text-muted outline-none rounded-xl"
                value={searchQuery}
                onChange={(e) => setSearchQuery(e.target.value)}
                onKeyDown={handleKeyDown}
              />
              {searchQuery && (
                ...
                >
                  <div className="bg-surface rounded-full p-1 text-muted border border-border">
            ...
            {searchQuery && (
              <div
                ...
                className="absolute bottom-full left-0 w-full mb-4 bg-elevated rounded-2xl shadow-premium border border-border overflow-hidden max-h-[45vh] overflow-y-auto custom-scrollbar animate-in slide-in-from-bottom-4 fade-in"
              >
                {filteredSearchResults.length > 0 ? (
                    ...
                    {filteredSearchResults.map((country) => {
                      ...
                      return (
                        <li key={country.id}>
                          <button
                            onClick={() => handleResultClick(country)}
                            className="w-full text-left px-6 py-4 flex items-center gap-4 transition-all border-b border-border last:border-none active:bg-surface uppercase tracking-tighter"
                          >
                            ...
                            <div>
                              <p
                                className={`text-xs font-black ${nameClass} ${glowClass}`}
                              >
                                {country.name}
                              </p>
                              <p className="text-[9px] font-bold text-muted tracking-widest">
                                {country.region}
                              </p>
                            </div>
                          </button>
                  ...
                  <div className="p-10 text-center text-[10px] text-muted font-black uppercase tracking-[0.3em] relative z-10">
          ...
          <div
            ref={regionScrollRef}
            className="w-full overflow-x-auto no-scrollbar relative z-10"
 ...
 ${
   showTerritories
     ...
     : "bg-surface text-muted border-border active:bg-surface"
 }
 `}
              >
 ...
 `}
              ...
              <div className="w-[1px] h-6 bg-surface flex-shrink-0 mx-1"></div>
              {REGIONS.map((region) => (
                <button
                  key={region}
                  data-region={region}
