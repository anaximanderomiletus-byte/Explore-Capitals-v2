...
const GAME_PATHS: Record<string, string> = {
  ...
  '10': 'territory-titan', '11': 'area-ace', '12': 'currency-craze',
    ...
    <main className="relative flex-grow bg-transparent w-full home-glow overflow-x-hidden select-none">
      ...
      <section className="relative overflow-hidden isolate w-full min-h-screen flex flex-col items-center justify-center pt-12 pb-24">
        ...
        <div className="relative w-full h-[320px] sm:h-[400px] md:h-[440px] flex items-center justify-center">
          ...
          >
            {/* Panel 0: Games */}
            <Panel index={0} activeIndex={activeIndex} scrollX={x} width={panelWidth} onClick={() => setActiveIndex(0)}>
              <VerticalContent items={featuredGames} type="games" />
            </Panel>

            {/* Panel 1: Globe */}
            <Panel index={1} activeIndex={activeIndex} scrollX={x} width={panelWidth} onClick={() => setActiveIndex(1)}>
                ...
                <div className="relative w-[300px] h-[300px] sm:w-[360px] sm:h-[360px] md:w-[420px] md:h-[420px] flex-shrink-0">
                  <div className="w-full h-full animate-breathe">
                    <Link 
                      to="/map" 
                      onClick={(e) => {
                        if (activeIndex !== 1) e.preventDefault();
                      }}
            ...
            {/* Panel 2: Countries */}
            <Panel index={2} activeIndex={activeIndex} scrollX={x} width={panelWidth} onClick={() => setActiveIndex(2)}>
              <VerticalContent items={COUNTRY_HIGHLIGHTS} type="countries" />
            </Panel>
          </motion.div>
        </div>
