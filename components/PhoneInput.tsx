...
const PHONE_FORMATS: Record<
  string,
  { pattern: RegExp; format: (digits: string) => string; maxLength: number }
> = {
  US: {
...
const PhoneInput: React.FC<PhoneInputProps> = ({
  ...
  const selectedCountry = useMemo(() => {
    ...
    return (
      sorted.find((c) => value.startsWith(c.dial_code)) || COUNTRY_CODES[0]
    );
  }, [value]);
  ...
  const handleNumberChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    const rawValue = e.target.value
      .replace(/[^\d]/g, "")
      .slice(0, formatConfig.maxLength);
  ...
  const smartPlaceholder = useMemo(() => {
    ...
    return (
      placeholders[selectedCountry.code] ||
  ...
  return (
    <div className="space-y-2">
      {label && (
        <label className="text-[10px] font-black uppercase tracking-[0.15em] text-muted ml-1 block">
          {label}
        </label>
      )}
      <div className="relative group">
        <div
          className={`flex bg-elevated border ${error ? "border-red-500" : "border-border"} rounded-xl shadow-inner focus-within:border-primary/50 transition-all overflow-hidden group-hover:border-border`}
        >
          {/* Country Selector Trigger */}
          <button
            type="button"
            onClick={() => setIsOpen(!isOpen)}
            className="flex items-center gap-2 px-4 py-3 border-r border-white/5 hover:bg-surface transition-colors shrink-0"
          >
          ...
          <input
            ...
            required={required}
            className="flex-1 bg-transparent px-4 py-3 text-sm font-medium text-text placeholder:text-muted outline-none"
          />
        </div>
        ...
        {isOpen && (
          <>
            ...
            />
            <div className="absolute top-full left-0 mt-2 w-full max-w-[320px] bg-surface-dark border border-border rounded-xl z-[2101] overflow-hidden backdrop-blur-2xl shadow-2xl">
              <div className="p-3 bg-surface border-b border-border">
                <div className="relative">
                  ...
                  />
                  <input
                    className="w-full bg-surface border border-border rounded-lg pl-9 pr-3 py-2.5 text-sm text-text placeholder:text-muted outline-none focus:border-primary/30"
              ...
              <div className="max-h-64 overflow-y-auto custom-scrollbar">
                {filteredCountries.map((c) => (
                  <button
                    ...
                    className={`w-full flex items-center justify-between px-4 py-3 text-sm transition-all ${selectedCountry.code === c.code ? "bg-accent-soft text-text" : "text-muted hover:bg-surface"}`}
                  >
                    ...
                    <span
                      className={`text-xs font-medium ${selectedCountry.code === c.code ? "text-primary" : "text-muted"}`}
                    >
                      {c.dial_code}
                    </span>
                  </button>
